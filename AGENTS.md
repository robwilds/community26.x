...
- **JS frontend** (`mgr/static/index.html`): 2860 lines, vanilla JS, dark theme CSS vars, zero deps. Features a header with Alfresco/Kibana service links, global controls, a dark/light mode toggle with moon/sun icon, and a version-sync flow.
- **Version sync**: The header **🔄 Update Versions** button syncs `docker-compose.yaml`; the **Compose Config** tab (in Available Files) syncs `commons/base.yaml`. Both fetch the upstream Alfresco templates (`community-compose.yaml`, `commons/base.yaml` from `raw.githubusercontent.com/Alfresco/acs-deployment/master`), diff `image:` lines of name-matched services, render a selectable side-by-side diff table, and write only user-applied rows. Backed by `GET`/`POST /api/compose` (filename whitelist: `docker-compose.yaml`, `commons/base.yaml`). Images are compared after stripping the `docker.io/` prefix so identical refs are not flagged.
- **Version rollback**: Every compose write records a journal entry in `mgr/data/compose_rollback.json` (capped at 20) containing a full `prevContent` snapshot plus per-service `changes` (`{service, before, after}`). `GET /api/rollback` lists history (shown via the Compose tab's **↩ Rollback** button); `POST /api/rollback` restores one service (`services: [name]`) by locating its `image:` line in the *current* file, or reverts the whole entry's `prevContent` snapshot when `services` is omitted. Rolled-back services are pruned from the entry. Line lookup uses `_find_service_image_line()` (server) / `findServiceImageLine()` (client).
